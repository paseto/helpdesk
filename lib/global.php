...
function aaModelInsertRequest($u, $ue, $group, $priority, $subject, $message, $files, $customfield = NULL, $code = NULL)
{
    ...
    if (empty($form_error) && (empty($file_error))) {
        ...
        //Custom
        $sqlCompany = 'SELECT UID,Pwd,Fname,Email,Signature,Role,Preferred_View,Layout_Style, Disabled, razao_social FROM ' . $pdo_t['t_users'] . ' t1 INNER JOIN empresas t2 ON t1.cnpj=t2.cnpj WHERE Email = :email AND Role = 0';
